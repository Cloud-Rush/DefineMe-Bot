...
os.environ['MERRIAM_WEBSTER_COLLEGIATE_KEY'] = "8a6a28c0-9617-42cc-b5c0-37e8f920e409"
...
def lookup(dictionary_class, key, query):
    ...
    if defs == []:
        return "No definitions found for '{1}'".format(dname, query)
    for word, pos, definition in defs:
        string +=  "{1} [{2}]: {3}\n\n".format(dname, word, pos, definition)
        #print string
    string += "http://www.merriam-webster.com/dictionary/{1}".format(dname, word, pos, definition)
    return string
